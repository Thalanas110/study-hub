...
export const Route = createFileRoute("/_authenticated/groups")({
  head: () => ({ meta: [{ title: "Browse groups — Studyhive" }] }),
...
function GroupsPage() {
  ...
  const { data: groups, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      // join profiles manually since cross-table FK isn't declared
      return data ?? [];
    },
  });
  ...
  return (
    <main className="mx-auto max-w-6xl px-6 py-12">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <h1 className="font-display text-4xl font-bold">Study groups</h1>
          <p className="mt-1 text-muted-foreground">Find your people. Join freely — no approval needed.</p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="mt-10 grid gap-5 md:grid-cols-2 lg:grid-cols-3">
          ...
          {groups?.length === 0 && (
            <p className="text-muted-foreground">No groups yet. Be the first to host one.</p>
          )}
        </div>
      )}
    </main>
